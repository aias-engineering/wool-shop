...
import { ErrorInCosmosDbAccess, MultipleUsersWithEmailFound, UserWithEmailNotFound } from "@/lib/server/core/failure";

function users(): Promise<Container> {
  return woolshopDatabase()
    .then(
      database => database.containers.createIfNotExists({
        id: 'users',
        partitionKey: 'id'
      })
    )
      ...
      users()
        ...
        .then((users) => 
          match(users)
            ...
            .otherwise((users) => MultipleUsersWithEmailFound(email, users.map(x => x.id)))
          )
        .catch(
          (reason) => ErrorInCosmosDbAccess(reason)
        )
}
