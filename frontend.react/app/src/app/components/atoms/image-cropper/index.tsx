...
export default function ImageCropper({uploadedImageAtom, onImageCropped}: Props) {
  ...
  async function handleImageCrop() {
    ...
    const croppedImageAtom = atom<CroppedImage|'Crop failed'>((get) => {
        if (blob){
          return {data: blob, name: get(uploadedImageAtom).name } as CroppedImage
        }
        else {
          return 'Crop failed';
        }
  ...
  return (
    <OverlayContainer>
      <NaqabCropper imageUrl={uploadedImage.dataUrl} 
                    onCropComplete={async(area) => await setCroppedAreaPixels(area)} />
      <Overlay>
        <Button onClick={async () => { await handleImageCrop()}}>
          finish
        </Button>
      </Overlay>
    </OverlayContainer>
