...
test.each([undefined, null, ''])(
  'with { name: %s } should return validation error',
  async (name) => {
...
test.each([undefined])(
  'with { description: %s } should return validation error',
  async (description) => {
    const formData = createFormData({
      ...woolySheep,
      description,
...
test.each([undefined, null, '', 'string', '11,00'])(
  'with { price: %s } should return validation error',
  async (price) => {
    const formData = createFormData({
      ...woolySheep,
      price,
...
test.each([undefined, null, ''])(
  'with { image: %s } should return validation error',
  async (image) => {
    const formData = createFormData({
      ...woolySheep,
      image,
...
function createFormData(input: object): FormData {
  ...
  Object.keys(input).forEach((property) => {
    ...
    if (value) form.append(property, value)
  })
  return form
}
